...
from Zhang_AITHU_SJTU_task1.models.helpers.utils import make_divisible
...
class Network(nn.Module):
    ...
    def _forward_conv(self, x):
        x = self.in_c(x)  # expand channel to 32, stride with 4
        x = self.stages(x)
        return x

    def forward(self, x):  # [bs, 1, 256, 65]
...
class Block2(nn.Module):
    def __init__(
        ...
        # create the three factorized convs that make up the inverted bottleneck block
        self.exp_conv = Conv2dNormActivation(in_channels,
                                             exp_channels,
                                             kernel_size=1,
                                             stride=1,
                                             norm_layer=nn.BatchNorm2d,
        ...
        # depthwise convolution with possible stride
        self.depth_conv = Conv2dNormActivation(exp_channels,
                                               exp_channels,
                                               kernel_size=(3, 1),
                                               stride=stride,
                                               padding=(1, 0),
        ...
        self.depth_conv2 = Conv2dNormActivation(exp_channels,
                                                exp_channels,
                                                kernel_size=(1, 3),
                                                stride=stride,
                                                padding=(0, 1),
        ...
        self.proj_conv = Conv2dNormActivation(exp_channels,
                                              out_channels,
                                              kernel_size=1,
                                              stride=1,
                                              norm_layer=nn.BatchNorm2d,
    ...
    def block(self, x):
        x = self.exp_conv(x)
        x = self.depth_conv(x) + self.depth_conv2(x)
        x = self.proj_conv(x)
        return x
...
class Network2(nn.Module):
    ...
    def _forward_conv(self, x):
        x = self.in_c(x) + self.in_c2(x)  # expand channel to 32, stride with 4
        x = self.stages(x)
        return x
...
def get_model(n_classes=10, in_channels=1, base_channels=32, channels_multiplier=2.3, expansion_rate=3.0,
              n_blocks=(3, 2, 1), strides=None, model_arch='baseline', n_mels=256):
    """
    ...
    if model_arch == 'baseline2':
        m = Network2(model_config)
    else:
        m = Network(model_config)
    return m
