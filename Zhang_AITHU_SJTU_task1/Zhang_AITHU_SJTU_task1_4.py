...
def load_backbone_model(config=Config):
    ...
    with pkg_resources.path(ckpts, "c96_unpruned_backbone.ckpt") as ckpt_path:
...
def load_model(model_file_path: Optional[str] = None,
               maska_file_path: Optional[str] = None,
               maskb_file_path: Optional[str] = None) -> Baseline2:
    """
    ...
    if model_file_path is None:
        ...
        with pkg_resources.path(ckpts, "c96_unpruned_distilled_2.ckpt") as ckpt_path:
            model_file_path = str(ckpt_path)

    if maska_file_path is None:
        # Use default checkpoint from package resources
        with pkg_resources.path(ckpts, "c96_maska_2.pth") as mask_path:
            maska_file_path = str(mask_path)

    if maskb_file_path is None:
        # Use default checkpoint from package resources
        with pkg_resources.path(ckpts, "c96_maskb_2.pth") as mask_path:
            maskb_file_path = str(mask_path)

    backbone_model = load_backbone_model(config)
    maska = torch.load(maska_file_path, map_location="cpu")
    maskb = torch.load(maskb_file_path, map_location="cpu")
    dummy_input = torch.rand(config.batch_size, 1, 256, 65)  # [B, 1, n_mels, T]
    pruned_model = ModelSpeedup(backbone_model,
                                dummy_input, maskb).speedup_model()
    pruned_model = ModelSpeedup(pruned_model,
                                dummy_input, maska).speedup_model()
